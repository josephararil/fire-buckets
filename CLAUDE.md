...
**After any change, bump `APP_VERSION` in `engine.js` AND `SW_VERSION` in `sw.js`** (format: `YYYYMMDD.N`, e.g. `"20260508.0"`). The two strings must stay in sync — they live in separate execution contexts. Bumping only one does not invalidate the cache. Current version: `"20260529.2"`.
...
- `simulateAffordability(state, { funDelta, essentialsDelta, oneOff, exitPortfolio })` → `{ before, after, deltaMonths, oneOffMonths, verdict, isAccumulating }` — sandbox spending-impact engine. Computes before/after in terms of `monthlyExpenses`, `annualExpenses`, `surplusMonthly`, `fireTarget`, `monthsToFire`, `exitWR`, `exitZone`. One-off delay is `ceil(oneOff / aSurplus)` months (uses after-surplus so a combined fun+one-off scenario reflects the new spending level). **Phase-aware verdict**: for accumulating phases (`employed`, `coast_fire`, `barista_fire`) verdict is based on surplus coverage and timeline impact — NOT portfolio WR, which is only meaningful while actually drawing. Verdict logic: accumulating → `bad`/"No." if `aSurplus < 0` (income < expenses); `warn`/"Careful." if total delay > 12 months; else `good`/"Fine." Drawing phases (lean/full/sabbatical) → zone-based: `bad`/"No." if zone elevated/cut; `warn`/"Careful." if deltaMonths > 12 or zone worsened; else `good`/"Fine."
...
3. **Portfolio capacity + Affordability sandbox** — rendered as a **2-column grid on non-mobile** (`gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr"`), stacked on mobile.
   ...
   - Right: `AffordabilityCard` — two independent inputs: **Monthly fun budget** `NumberField` (seeded from `state.monthlyFunEUR`, +€100/+€200 chips) and **One-off purchase** `NumberField` (default 0, preset chips €500/€1k/€5k). Both can be active simultaneously. Calls `simulateAffordability` on every change. Shows phase-aware tone-colored verdict, then before→after grid: FIRE date, FIRE target, and either **monthly surplus** (accumulating phases) or **exit WR pill** (drawing phases) + total ±months delta. "Apply to Plan" button updates `monthlyFunEUR` in state (only when `funDelta !== 0`). **Never auto-mutates state** — sandbox only until Apply is clicked.
